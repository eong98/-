...
· 역할: 단독 메인 개발
· 담당: 관리자(DBMS) 페이지, 사용자 페이지, 상품 관리, 할인 구간 정책, 쿠폰, 게시판 전반
· 연동: 이카운트 ERP 연동으로 주문·재고·정산 데이터 자동 동기화
· 포인트: B2B 특성상 거래처별 단가/할인 구간이 달라, 할인 정책을 조건 분기로 하드코딩하지 않고
          [정책 테이블 기반 구조로 분리] 하여 신규 거래처 추가 시 코드 수정 없이 대응 가능하도록 설계
